...
public:
	...
	// template <typename F>
	vector< pair<Date, string> > FindIf(function < bool(const Date &date, const std::string &event) > pred);
	...
	int RemoveIf(function < bool(const Date &date, const std::string &event) > pred);
